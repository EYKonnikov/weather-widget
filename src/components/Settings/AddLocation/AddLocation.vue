  ...
  <div>
    ...
    >
      ...
      <button
        ...
        :tabindex="3"
        :disabled="
          !new_location.country.complete || !new_location.city.complete
        "
      >
        +
      </button>
  ...
  methods: {
    ...
    uniquenessValidation(locations, new_location) {
      return locations.filter(
        (e) =>
          e.city === new_location.city && e.country === new_location.country
      );
    },
    ...
    countryAutocomplete() {
      ...
      if (!this.isEmptyOrSpaces(country)) {
        for (const autocomplete_country in this.autocomplete_locations) {
          if (
            autocomplete_country.toLowerCase().includes(country.toLowerCase())
          ) {
            ...
            if (autocomplete_country.length != country.length) {
              ...
              this.new_location.country.text =
                country + autocomplete_country.slice(start_index, end_index);
    ...
    cityAutocomplete() {
      ...
      if (
        !this.isEmptyOrSpaces(this.new_location.country.text) &&
        !this.isEmptyOrSpaces(city)
      ) {
        for (const autocomplete_country in this.autocomplete_locations) {
          if (autocomplete_country == this.new_location.country.text) {
            for (const city_id in this.autocomplete_locations[
              autocomplete_country
            ]) {
              let autocomplete_city =
                this.autocomplete_locations[autocomplete_country][city_id];

              if (
                autocomplete_city.toLowerCase().startsWith(city.toLowerCase())
              ) {
                ...
                // Change city input
                this.new_location.city.text =
                  city + autocomplete_city.slice(start_index, end_index);
...
.settings__content__new_location__fields {
  width: 100%;
  margin-right: 20px;
}

.settings__content__new_location__fields > div {
  position: relative;
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
...
.settings__content__new_location__fields > div:first-child {
  margin-top: 0;
}

.settings__content__new_location__icon {
  position: absolute;
  top: 10px;
  left: 10px;
  width: 20px;
...
.settings__content__new_location__icon__error {
  fill: #db4437;
  -webkit-animation: shakeIcon 0.5s ease-in-out;
  animation: shakeIcon 0.5s ease-in-out;
}

.settings__content__new_location__input {
  width: 100%;
  height: 40px;
  padding: 5px 10px;
  padding-left: 40px;
...
.settings__content__new_location__input:focus {
...
.settings__content__new_location__input__error:focus {
  ...
  -webkit-animation: shakeInput 0.5s ease-in-out;
