<template>
  <div class="weather_board">
    <div v-for="item in weather" :key="item.id" class="weather_board__card">
      ...
      <!-- Feels like -->
      <p class="weather_board__card__description">
        Feels like {{ Math.round(item.main.feels_like) }}°C.
      </p>

      <!-- Descriptions -->
      <p
        class="weather_board__card__description"
      ...
      <img
        ...
        :src="
          require(`../../assets/img/weather/${item.weather[0].main.toLowerCase()}.jpg`)
        "
...
.weather_board__card:hover .weather_board__card__background_image {
...
.weather_board__card__location_title img {
  margin: auto 0;
}

/* Main temperature */
.weather_board__card__main_temperature {
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
  -webkit-box-pack: center;
...
.weather_board__card__main_temperature p {
  line-height: 100px;
  font-size: 48px;
}

/* Description */
.weather_board__card__description {
  text-align: center;
  text-shadow: 0px 0px 5px rgba(0, 0, 0, 0.25);
}

/* Divider */
.weather_board__card__divider {
  width: 100%;
  height: 1px;
...
.weather_board__card__other > div {
  width: 50%;
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
...
.weather_board__card__other svg {
  width: 20px;
  height: 20px;
  fill: white;
  margin-right: 5px;
